...
export default function App() {
        ...
        setQuestions(
          data.results.map((data) => ({
            ...
            answers: shuffle([
              ...
              ...decodeURIComponent(data.incorrect_answers).split(","),
            ]),
            selectedAnswer: "",
            isRevealed: false,
          }))
  ...
  React.useEffect(() => {
    ...
    if (quiz.isChecked) {
      document.querySelector(
        ".result"
      ).textContent = `You scored ${count}/5 correct answers`;
  ...
  function check() {
    setQuestions((prevQuestions) =>
      prevQuestions.map((question) => ({ ...question, isRevealed: true }))
    );
  ...
  return quiz.isStarted ? (
    <main className="quiz">
      {questions.length > 0 && <div>{constructQuestions()}</div>}
      <div className="action">
        <p style={{ display: "none" }} className="result"></p>
        <button
          className="btn check-answers-btn"
          onClick={!quiz.isChecked ? check : playAgain}
        >
          {" "}
          {!quiz.isChecked ? "Check answers" : "Play again"}
        </button>
      </div>
    </main>
  ) : (
    <div className="intro">
      ...
      <button className="btn intro-btn" onClick={startQuiz}>
        Start quiz
      </button>
    </div>
  );
